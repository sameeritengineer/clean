        ...
        <ul class="nav navbar-nav float-xs-right">
          <li class="dropdown dropdown-user nav-item">
            ...
            <div class="dropdown-menu dropdown-menu-right">             
              <a class="dropdown-item" href="{{ route('supervisior::logout') }}" onclick="event.preventDefault();document.getElementById('logout-form').submit();">{{ __('Logout') }}</a>
              <form id="logout-form" action="{{ route('supervisior::logout') }}" method="post" style="display: none;">@csrf</form>
            </div>
          </li>
        </ul>
      </div>
