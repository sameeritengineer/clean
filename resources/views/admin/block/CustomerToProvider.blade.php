...
@endif 
      ...
      <div class="content-header row">
        <div class="content-header-left col-md-6 col-xs-12 mb-2">
          <h3 class="content-header-title mb-0">Block Provider For Customer</h3>
      </div>  
  </div>
                ...
                <div class="card-body collapse in">
                  <div class="card-block card-dashboard">  
                   <div class="modal-header">
				        ...
				        <label class="modal-title text-text-bold-600" id="myModalLabel33">Block Provider</label>
		           </div> 
			            ...
			            <h5>Select Provider<span class="required"></span></h5>
			              <div class="controls">
			              	  <select class="select2 form-control block" id="responsive_single" name="provider_id" style="width: 100%">
			              	  	@php 
			              	  	 $p_role_id = App\Role::where('name','provider')->first();
                                 $providers = App\User::join('user_roles','user_roles.user_id','=','users.id')->where('user_roles.role_id',$p_role_id->id)->select('users.id as User_id','users.first_name as first_name')->get();
			              	  	@endphp
                   ...
                   <div class="table-responsive">                 
                    <table class="table table-striped table-bordered zero-configuration">
                      <thead>
                        <tr>
                          <th>#</th>
                          <th>Blocked by(Customer Name)</th>
                          <th>Blocked (Provider Name)</th>
                          <th>Status</th>
                        </tr>
                      </thead>
                      <tbody>
                        @foreach($blockedproviders as $b_provider)
                        <tr>                  
                          <td>{{$loop->iteration}}</td>
                          @php 
                            $custome_name = App\User::find($b_provider->customer_id);
                          @endphp
                          <td>{{$custome_name->first_name}}</td>
                          @php 
                            $provider_name = App\User::find($b_provider->provider_id);
                          @endphp
                          <td>{{$provider_name->first_name}}</td>
                          <td><button class="btn btn-primary" onclick="unblockprovider({{$b_provider->id}})">UnBlock</button></td>
                        </tr>
                        @endforeach
                      </tbody>
                    </table>
...
function unblockprovider(id)
{
  var url = "{{secure_url('serviceadmin/UnBlock-Provider')}}";
  $.ajax
  ({
    type: "post",   
    url: url,             
    ...
    success:function(data)
    {
      if(data == 1)
      {
        toastr.success("You'r successfully UnBlock Provider", "Great !");
